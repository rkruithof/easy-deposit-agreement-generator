...
class ServiceStarter extends Daemon with DebugEnhancedLogging {
  var app: EasyDepositAgreementGeneratorApp = _
  var service: EasyDepositAgreementGeneratorService = _

  override def init(context: DaemonContext): Unit = {
    logger.info("Initializing service...")
    val configuration = Configuration(File(System.getProperty("app.home")))
    app = new EasyDepositAgreementGeneratorApp(configuration)
    service = new EasyDepositAgreementGeneratorService(configuration.serverPort, Map(
